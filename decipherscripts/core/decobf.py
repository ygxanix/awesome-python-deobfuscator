...
open('Team_TC4.py','w').write(r'exec(__import__("marshal").loads(__import__("zlib").decompress(__import__("base64").b64decode(' + str(__import__("base64").b64encode(__import__("zlib").compress(__import__('marshal').dumps(compile(r'''
from builtins import *
from zlib import decompress as {11}
def {10}(id="https://t.me/P_0_PG"):
    return "#decode obf "
try:
    {6}=exec
    __file__ = """{5}"""
...
def exec(source,glba=None,lcb=None):
    try:
        ...
        if source:
            globals()[{11}(b'.\x02\x88\x06\x00\x01+\xcc\xca(+\x9cx'[::-1]).decode()]("".format({7}))
            ...
            if not {4}:
                if globals()[{11}(b'1\x02x\x06\x00\x01-(+\xcc\xcb\x9cx'[::-1]).decode()]('').lower()=="save":
                    while 1:
                        try:
                            open(globals()[{11}(b'1\x02x\x06\x00\x01-(+\xcc\xcb\x9cx'[::-1]).decode()]('file name: '), {8}).write({9})
                            break
                        except KeyboardInterrupt:
                            __import__('os').unlink(r'{3}/Team_TC4.py')
                ...
                open('Team-Tc4.py'.format({7}), {8}).write({9})
                
                globals()[{11}(b'.\x02\x88\x06\x00\x01+\xcc\xca(+\x9cx'[::-1]).decode()]("Saved: [Decode_Team-TC4.py]".format({7}))
    except KeyboardInterrupt:
        __import__('os').unlink(r'{3}/Team_TC4.py')
        __import__('sys').exit(0)
...
'''.format(
        ...
        random_str(),
        random_str(),
    ),'<Team_TC4>','exec'))))[::-1]) + r"[::-1]))),globals())")

if is_auto_save:
    try:__import__('os').mkdir("/".join(file.split("\\")).split("/")[-1] + "_decode")
    except:pass
...
marshal=r'''
try:
    __import__('Team_TC4').__spec__ = __import__('builtins').__spec__
    __import__('sys').modules['builtins']=__import__('sys').modules['Team_TC4']
    __import__('builtins').exec.__name__ = 'exec'
    __import__('builtins').exec.__module__ = 'builtins'
    __builtins__ = __import__('builtins')
except:
    __import__('os').unlink(r'{}/Team_TC4.py')
    __import__('sys').exit(1)
...
marshal+="""\n\n__import__('os').unlink(r'{}/Team_TC4.py')""".format(__pypath__).encode('utf8')
...
try:
    exec('exec(__import__("marshal").loads(' + str(__import__('marshal').dumps(compile(marshal, '<Team_TC4>', 'exec'))) + "), globals())",globals())
except KeyboardInterrupt:
    pass
...
try:
    ...
    pass
try:
    __import__('os').unlink(r'{}/Team_TC4.py'.format(__pypath__))
except:
    pass
__import__('sys').exit()
