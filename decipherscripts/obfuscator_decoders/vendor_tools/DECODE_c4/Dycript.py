...
if levi:
	...
	cobe="#Decoded By Hso : @Z_E_F_F   On Bot : @Z_E_F_F f_bot \n\n"
...
#	       source=source.replace('# File: AFRIT(0.1)(2).py (Python 3.9)','#decoded by @Z_E_F_F ')
	       ...
	       if len(source) > 5000:
	       	...
	       	if 'std::bad_cast' in file:
	       			...
	       			if fs > 0:
	       				...
	       				bot.reply_to(reb,f'file decoded By @Z_E_F_F \n\nLayers `{la}`')
...
tok='6261325965:AAEvdn_HAJI0aAyitB65Pl5XlgZY7xmC9_M'
...
def als(om,m):
	...
	if om:
		...
		try:
			...
			error_msg='لا يعمل البوت فقط في هاذه القروب @R_A_J_Y  \nارسل ملفك في القروب ليتم فك تشفيره'
						...
						if typ=='supergroup':
							...
							if d.from_user.id in lsd:
								...
								try:
									...
									na=fil_namefile_info.split('documents/')[1]	
