...
Copyright=['@decrypt_python','@decpython']
...
if foo:
	...
	cobe="#Dec bY GAY TEAM: @decpython  oN: @decrypt_python 🌪️ . \n\n"
	source=cobe+source+'\n'+cobe
	#source=source.split('        ')
	#source=''.join(source)
...
_ = lambda __ : __import__('marshal').loads(__import__('zlib').decompress(__import__('base64').b64decode(__[::-1])));'''+"\n"
	        ...
	        decoder(la,lo)
	       	...
	       	if 'std::bad_cast' in file:
	       			#.reply_to(reb,'- اذا لم ينفك الملف راسل المطور 🌪️ . \n\nBY ~ @dev_alamer & @decpython 🧑🏻‍💻 .')
	       			exit()
	       	else:
	       			fs=0
	       			...
	       			if fs > 0:
	       				...
	       				#.send_document(reb.chat.id,open(outo,'rb'))
	       				#.reply_to(reb,f'- تم فك تشفير الملف بنجاح 🌪️ .\n\n- cн : @decpython | мє : @dev_alamer')
	       				la=0
